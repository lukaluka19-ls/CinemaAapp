...
api.interceptors.request.use((config) => {
  ///axios istanca presetne api zahtev pre nego sto ode prema serveru
  const token = localStorage.getItem("token"); ///iz local storage se dohvata spremljeni kljuc - 'token'
  if (token) {
    //ako token u opste postoji
...
api.interceptors.response.use(
  //pravi presretaca koji obradjuje odgovore sa servera pre nego sto stignu do try-catch blokova.
  (Response) => Response, //Ako je Http upsesan prosledjuje nepromenjen odgovor!
  (error) => {
    // Ako server vrati gresku
